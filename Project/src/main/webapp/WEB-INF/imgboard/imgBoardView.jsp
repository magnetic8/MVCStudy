	    			...
	    			<c:when test="${mvo.u_type eq 'admin' }">
	    				<c:choose>
			    			<c:when test="${vo.notice eq 0 }">
			    				<button class="btn btn-sm btn-warning" onclick="goNotice(${vo.img_seq})">공지</button>
			    			</c:when>
			    			<c:otherwise>
			    				<button class="btn btn-sm btn-warning" onclick="delNotice(${vo.img_seq})">공지 제거</button>
			    			</c:otherwise>
	    				</c:choose>
	    				<button class="btn btn-sm btn-warning" onclick="goUpdate(${vo.img_seq})">수정</button>
	    				<button class="btn btn-sm btn-danger" onclick="goDel(${vo.img_seq})">삭제</button>
	    			</c:when>
