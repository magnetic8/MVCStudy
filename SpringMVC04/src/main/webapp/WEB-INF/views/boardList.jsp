    			...
    			<table class="table table-hover table-bordered">
    				...
    				<div style="text-align: center;">
    						...
    						<c:if test="${pageMaker.prev }">
    							<li><i class="fa"><a href="javascript:goPage(${pageMaker.startPage-1})">◀</a></i></li>    					
    						</c:if>
    						
    						<!-- 페이징 처리 -->
    							<c:forEach var="pageNum" begin="${pageMaker.startPage }" end="${pageMaker.endPage }">
    							 <c:if test="${pageMaker.cri.page==pageNum }">
    								<li><i class="fa"><a href="javascript:goPage(${pageNum})"><font color="red">${pageNum }</font></a></i></li>
    							 </c:if>
    							 <c:if test="${pageMaker.cri.page!=pageNum }">
    								<li><i class="fa"><a href="javascript:goPage(${pageNum})">${pageNum }</a></i></li>
    							 </c:if>
    							</c:forEach>
    							<!-- 다음 페이지 -->
    						<c:if test="${pageMaker.next}">
    							<li><i class="fa"><a href="javascript:goPage(${pageMaker.endPage+1})">▶</a></i></li>    					
    						</c:if>
    						</ul>
    					 </div>	
    	</div>
